...
def rm_adw(self):
    try:
        if os.path.exists('%s/ADWLauncher.apk' % SystemApp) == True:
            ...
            EdsNotify().run("Move successful", "File is now in 'Removed_Apps' so you can restore if needed") 
        elif os.path.exists('%s/ADWLauncher.apk' % Removed) == True:
            print 'Already Removed'
            EdsNotify().run("File Was already removed", "File is in 'Removed_Apps' so you can restore if needed")
        else: 
            EdsNotify().run("Move Unsuccessfull", "File Could Not Be Found: 'Guess I can't move something thats not found'")
    except:
        print 'bad stuff happened... Trying to fix the problem'
        os.remove('%s/ADWLauncher.apk' % Removed)
        EdsNotify().run("Failed Please Try Again", "Second Try Should have fixed the issue.\nIf Not There is a Issue please submit Bug")       
...
def clean_removed(self):
    try:
        if os.listdir(Removed):
            ...
            EdsNotify().run('Clean Complete', "'Removed_Apps' Has Been Cleaned")
        else:
...
def restore_removed(self):
    for f in os.listdir(Removed):
        ...
        EdsNotify().run("Removed Apps Restored", "All of the apps you removed have been restored")
